...
def loss(emp_cov, x, alpha):
    loss = np.sum(emp_cov * x) - np.linalg.slogdet(x)[1] + alpha * l1_penal_off_diag(x)
    return loss
...
def prox_map(emp_cov, x, step_size, alpha):
    ...
    return soft_threshold(x - step_size * grad(emp_cov, x), step_size * alpha)
...
class ProxGrad:
    ...
    def fit_MFISTA(self, emp_cov, alpha, step_size=0.25, tol=1e-8, dg=None, F=None):
